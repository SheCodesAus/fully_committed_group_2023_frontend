            ...
            <table id="sessions-table">
                ...
                <tbody className="sessions-list-table">
                    {annotatedSessionData?.map(session => (
                        <tr key={session.id}>
                            <td id="session-name" className="hide"><Link to={`/sessions/${session.id}`}>Session: {new Date(session.start_date).toLocaleDateString('en-AU', { day: 'numeric', month: 'short', year: '2-digit' })} @ {new Date(session.start_date).toLocaleTimeString([], {hour: '2-digit', minute: '2-digit'})}</Link> </td>
                            <td className="hide2" id="session-date"><Link to={`/sessions/${session.id}`}>{new Date(session.start_date).toLocaleDateString('en-AU', { day: 'numeric', month: 'short', year: '2-digit' })}</Link></td>
                            <td className="hide2">
                                {" "}
